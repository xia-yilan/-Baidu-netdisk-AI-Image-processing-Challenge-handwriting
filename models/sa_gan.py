...
cur_block = cur_block.create_var(name = 'X',
                                    shape = [-1, 23, 48],
                                    dtype = 'float32')
'''
...
class ASPP(nn.Layer):
    ...
    def forward(self, x):
        ...
        net = self.conv_1x1_output(paddle.concat(
                                        x=[image_features, atrous_block1, atrous_block6,
                                        atrous_block12, atrous_block18], axis=1))
        return net

class STRNet(nn.Layer):
    ...
    def forward(self, x):
       ...
       # import pdb;pdb.set_trace()
        ...
        #upsample
        x = self.deconv1(x)
        x = paddle.concat(x=[self.lateral_connection1(con_x4), x], axis=1)
        x = self.deconv2(x)
        x = paddle.concat(x=[self.lateral_connection2(con_x3), x], axis=1)
        x = self.deconv3(x)
        xo1 = x
        x = paddle.concat(x=[self.lateral_connection3(con_x2), x], axis=1)
        x = self.deconv4(x)
        xo2 = x
        x = paddle.concat(x=[self.lateral_connection4(con_x1), x], axis=1)
        #import pdb;pdb.set_trace()
        x = self.deconv5(x)
        ...
        ### mask branch ###
        mm = self.mask_deconv_a(paddle.concat(x=[x_mask,con_x4],axis=1))
        mm = self.mask_conv_a(mm)
        mm = self.mask_deconv_b(paddle.concat(x=[mm,con_x3],axis=1))
        mm = self.mask_conv_b(mm)
        mm = self.mask_deconv_c(paddle.concat(x=[mm,con_x2],axis=1))
        mm = self.mask_conv_c(mm)
        mm = self.mask_deconv_d(paddle.concat(x=[mm,con_x1],axis=1))
        ...
        x = self.coarse_deconva(paddle.concat(x=[x, x_c2,self.c2(con_x2)],axis=1))
