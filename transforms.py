...
class Compose:
    ...
    def __call__(self, input, gt, mask):
        ...
        if isinstance(mask, str):
            mask = cv2.imread(mask).astype('float32') #, cv2.IMREAD_GRAYSCALE
...
def sample_images(epoch, i, output, gt, input, args):
    ...
    output = output.cast('float64') #int64
    gt = gt.cast('int64')
...
def resize(im, target_size=608, interp=cv2.INTER_CUBIC):
    ...
    im = cv2.resize(im, (2048, 2048), interpolation=interp)
    return im
...
class Resize:
    ...
    def __init__(self, target_size=(512, 512), interp='CUBIC'):
        self.interp = interp
    ...
    def __call__(self, im1, im2, im3):
        ...
        ih, iw = im1.shape[0], im1.shape[1]
        (tw, th) = self.target_size

        if ih>2048 and iw>2048:
            im1 = cv2.resize(im1, dsize=(1024,1024), interpolation=cv2.INTER_CUBIC)
            im2 = cv2.resize(im2, dsize=(1024,1024), interpolation=cv2.INTER_CUBIC)
            im3 = cv2.resize(im3, dsize=(1024,1024), interpolation=cv2.INTER_NEAREST)
            ih, iw = im1.shape[0], im1.shape[1]

        # if ih<1024 and iw<1024:
        #     im1 = cv2.resize(im1, (512, 512), interpolation=cv2.INTER_CUBIC)
        #     im2 = cv2.resize(im2, (512, 512), interpolation=cv2.INTER_CUBIC)
        #     im3 = cv2.resize(im3, (512, 512), interpolation=cv2.INTER_NEAREST)
        #     ih, iw = 512, 512


        ix = random.randrange(0, iw - tw + 1)
        iy = random.randrange(0, ih - th + 1)

        im1 = im1[iy:iy + th, ix:ix + tw, :]
        im2 = im2[iy:iy + th, ix:ix + tw, :]
        im3 = im3[iy:iy + th, ix:ix + tw, :]
